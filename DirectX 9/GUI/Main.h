...
public:
	void Initialize( ImGuiIO& io, LPDIRECT3DDEVICE9 _device ) {
		...
		D3DXCreateTextureFromFileInMemory( _device, ( void* )logo_binary, sizeof( logo_binary ), &logo );
		D3DXCreateTextureFromFileInMemory( _device, ( void* )logo_big_binary, sizeof( logo_big_binary ), &logo_big );
	...
	void Render( ) {
		...
		ImGui::Begin( "UI", 0, ImGuiWindowFlags_NoDecoration );
		{
			...
			ImGui::BeginChild( "header", { -1, 40 }, 0, ImGuiWindowFlags_NoBackground );
			{
				...
				ImGui::GetWindowDrawList( )->AddText( ImGui::GetIO( ).Fonts->Fonts[1], ImGui::GetIO( ).Fonts->Fonts[1]->FontSize, { 45, ImGui::GetWindowHeight( ) / 2 - ImGui::GetIO( ).Fonts->Fonts[1]->FontSize / 2 }, ImGui::GetColorU32( ImGuiCol_Text ), "FLY" );
				ImGui::GetWindowDrawList( )->AddText( ImGui::GetIO( ).Fonts->Fonts[1], ImGui::GetIO( ).Fonts->Fonts[1]->FontSize, { 45 + ImGui::GetIO( ).Fonts->Fonts[1]->CalcTextSizeA( ImGui::GetIO( ).Fonts->Fonts[1]->FontSize, FLT_MAX, -1, "FLY" ).x, ImGui::GetWindowHeight( ) / 2 - ImGui::GetIO( ).Fonts->Fonts[1]->FontSize / 2 }, ImGui::GetColorU32( ImGuiCol_Scheme ), "SIDE" );
